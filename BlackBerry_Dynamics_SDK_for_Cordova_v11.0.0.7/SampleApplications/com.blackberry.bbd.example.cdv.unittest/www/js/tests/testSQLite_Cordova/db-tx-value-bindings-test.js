...
var mytests = function() {
  ...
  for (var i=0; i<scenarioCount; ++i) {
    ...
    // skip plugin test on 'Plugin-implementation-default' if it's android blackberry plugin
    if (isAndroid && isBlackBerryPlugin && (i === 0)) continue;
              ...
              tx.executeSql("INSERT INTO test_table VALUES (X'40414243')", [], function(ignored, rs1) {
                ...
                tx.executeSql('SELECT HEX(data) AS hexValue FROM test_table', [], function(ignored, rs2) {
                  ...
                  tx.executeSql('SELECT * FROM test_table', [], function(ignored, rs3) {
                    ...
                    if (!isWebSql && !isWindows && isAndroid && isImpl2 && !isBlackBerryPlugin) expect('PLUGIN BEHAVIOR CHANGED for android.database implementation').toBe('--');
                    expect(rs3).toBeDefined();
                    expect(rs3.rows).toBeDefined();
                    ...
                    else if (!isWebSql && isAndroid && isBlackBerryPlugin)
                      expect(item.data).toBe('QEFCQw==\u000a'); // (base64)
                    else
              ...
              tx.executeSql("INSERT INTO test_table VALUES (X'FFD1FFD2')", [], function(ignored, rs1) {
                ...
                tx.executeSql('SELECT HEX(data) AS hexValue FROM test_table', [], function(ignored, rs2) {
                  ...
                  tx.executeSql('SELECT * FROM test_table', [], function(ignored, rs3) {
                    ...
                    if (!isWebSql && isAndroid && isImpl2 && !isBlackBerryPlugin) expect('PLUGIN BEHAVIOR CHANGED: UNEXPECTED SUCCESS on Android with androidDatabaseImplementation: 2 setting').toBe('--');
                    expect(rs3).toBeDefined();
                    expect(rs3.rows).toBeDefined();
                    expect(rs3.rows.length).toBeDefined();
                    ...
                    if (!isWebSql && (isAppleMobileOS || isMac)) {
                      ...
                      if (!isWebSql && isAndroid && isBlackBerryPlugin)
                        expect(mydata.length).toBe(9); // (base64)
                ...
                tx.executeSql('SELECT HEX(name) AS hexValue FROM test', [], function (tx_ignored, rs1) {
                  ...
                  tx.executeSql('SELECT name FROM test', [], function (tx_ignored, rs) {
                    ...
                    if ((isWebSql && isAndroid &&
                        ...
                        (!isWebSql && isAndroid && isBlackBerryPlugin) ||
                        (!isWebSql && isWindows)) {
