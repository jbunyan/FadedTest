...
describe('File API', function () {
    ...
    var MEDIUM_TIMEOUT = 15000;
                            ...
                            srcDirectory.getDirectory(srcDirNestedSecond, { create: true }, function () {
                                srcDirectory.moveTo(dstDirectory, srcDir, function successMove (transferredDirectory) {
                                    ...
                                    directoryReader.readEntries(function successRead (entries) {
                                        ...
                                        if (!isChrome) {
                                            // DEVNOTE: Need sort because of each platform return
                                            // sorted entries array in own way
                                            sortedEntries = entries.sort(sortByName);

                                            expect(sortedEntries[0].name).toBe(srcDirNestedFirst);
                                            expect(sortedEntries[1].name).toBe(srcDirNestedSecond);
                                        }
                                        deleteEntry(dstDir, done);
            ...
            it('file.spec.109 should be able to resolve a file:/// URL', function (done) {
                ...
                root.getFile(localFilename, {
                    ...
                    */
                    ...
                    }, done, 'BBDFile', '_getLocalFilesystemPath', [entry.toURL()]);
        ...
        describe('resolveLocalFileSystemURL on cdvfile://', function () {
            xit('file.spec.147 should be able to resolve cdvfile applicationDirectory fs root', function (done) {
        ...
        if (cordova.platformId === 'android') { // eslint-disable-line no-undef
            describe('content: URLs', function () {
                ...
                it('file.spec.138 copyTo: content', function (done) {
                    testContentCopy('content://com.blackberry.bbd.cordova.plugins.file.testprovider/www/fixtures/asset-test/asset-test.txt', done);
                });
                it('file.spec.139 copyTo: content /w space and query', function (done) {
                    testContentCopy('content://com.blackberry.bbd.cordova.plugins.file.testprovider/?name=foo%20bar&realPath=%2Fwww%2Ffixtures%2Fasset-test%2Fasset-test.txt', done);
                });
                it('file.spec.140 delete: content should fail', function (done) {
                    resolveLocalFileSystemURL('content://com.blackberry.bbd.cordova.plugins.file.testprovider/www/fixtures/asset-test/asset-test.txt', function (entry) { // eslint-disable-line no-undef
